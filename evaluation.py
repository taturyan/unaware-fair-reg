from data_prep import get_LawSchool_data, get_frequencies
...
def get_stats(dataset='lawschool', num=10, T=15000, eps = [0.01, 0.01], beta='auto', L='auto',
    ...
    for i in range(1, num+1):
        ...
        if data=='lawschool':
            reg = xgb.XGBRegressor(max_depth=10, objective='reg:linear', n_estimators=400, reg_lambda=1, gamma=2)
            clf = xgb.XGBClassifier(max_depth=5, objective='binary:logistic', n_estimators=200, gamma=2)
        ...
        for s in range(K):
            ...
            base_DP_arr[s].append(base_DP[s])

        #saving the arrays for reproducing
        np.save('./saved_arrays/'+data+'_max_num_'+str(i), max_arr)
        np.save('./saved_arrays/'+data+'_avg_num_'+str(i), avg_arr)
        np.save('./saved_arrays/'+data+'_sum_num_'+str(i), sum_arr)
        np.save('./saved_arrays/'+data+'_std_num_'+str(i), std_arr)
        np.save('./saved_arrays/'+data+'_DP_num_'+str(i), DP_arr)
        np.save('./saved_arrays/'+data+'_risk_num_'+str(i), risk_history)
        np.save('./saved_arrays/'+data+'_base_DP_num_'+str(i), base_DP_arr)
        np.save('./saved_arrays/'+data+'_base_risk_'+str(i), base_risk_all)

        print ('retrieving training history retrieved and saved')
        print ('===============================================')
        
        
        return risk_history, max_all, avg_all, sum_all, std_all, DP_all, base_risk_all, base_DP_all
